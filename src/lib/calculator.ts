...
export type ViewingPreset = (typeof VIEWING_PRESETS)[number];
...
const ASPECT_RATIO_PRESETS: ReadonlyArray<
  (typeof ASPECT_RATIOS)[number] & { value: readonly [number, number] }
> = [
  { label: "1:1", value: [1, 1] as const },
  { label: "2:1", value: [2, 1] as const },
  { label: "21:9", value: [21, 9] as const },
...
export function getCroppedDimensions(
  ...
  if (ratio === "nocrop" || !ratio || origW <= 0 || origH <= 0)
